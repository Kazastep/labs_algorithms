...
def self_check() -> None:
    ...
    # Сумма пустого массива всегда равна 0
    assert array_sum([]) == 0

    # Сумма массива из одного элемента равна этому элементу
    assert array_sum([7]) == 7

    # Максимум массива из одного элемента равен этому элементу
    assert array_max([5]) == 5

    # Максимум должен корректно находиться среди отрицательных чисел
    assert array_max([-10, -3, -7]) == -3

    # Если все элементы различны, равных пар нет
    assert count_equal_pairs([1, 2, 3, 4]) == 0

    # Если все n элементов одинаковы, число пар равно n(n - 1) / 2
    a = [5, 5, 5, 5]
    n = len(a)
    assert count_equal_pairs(a) == n * (n - 1) // 2

    # Любое число в степени 0 равно 1
    assert binary_pow(7, 0) == 1

    # Проверка бинарного возведения по модулю
    assert binary_pow(2, 10, mod=5) == pow(2, 10, 5)
